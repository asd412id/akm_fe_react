...
import { MdArrowBack, MdArrowForward, MdDoneAll } from 'react-icons/md';
      ...
      <AuthPeserta title={'Mengerjakan Soal: ' + (dataUjian ? dataUjian?.jadwal?.name : '...')} error={error}>
        ...
        <div className="flex-w-full flex-col gap-3">
          ...
          <div className="flex flex-col md:flex-row justify-center gap-3 mt-5">
            <div className="flex justify-between md:justify-center gap-3">
              <Button className='flex items-center rounded-full shadow-md shadow-gray-400' onClick={() => {
                if (number > 0) {
                  ...
                  setNumber(dataUjian.peserta_tests.length - 1);
                }
            ...
            {number >= dataUjian.peserta_tests.length - 1 &&
              <Button color={'failure'} className='rounded-full shadow-md shadow-gray-400 flex items-center' onClick={() => setOpenDialog(true)}>SELESAI UJIAN <MdDoneAll className='w-5 h-5 ml-2' /></Button>
            }
          </div>
        </div>
      </AuthPeserta>
